 ...
 */
template<class T>
Matrix3<T> getRotationMatrixX(T th){
 ...
 */
template<class T>
Matrix3<T> getRotationMatrixY(T th){
    return Matrix3<T>(cos(th),0,sin(th),
                      0,1,0,
                      -sin(th),0,cos(th));
}
/**
 ...
 */
template<class T>
Matrix3<T> getRotationMatrixZ(T th){
    return Matrix3<T>(cos(th),-sin(th),0,
                      sin(th), cos(th),0,
                      0,       0,1);
}
/**
 ...
 */
template<class T> Matrix3<T> getRotationMatrixXYZ(const Vector3<T>& angle){    return getRotationMatrixX(angle[0])*getRotationMatrixY(angle[1])*getRotationMatrixZ(angle[2]);}
template<class T> Matrix3<T> getRotationMatrixXYZ(const T& ax, const T& ay, const T& az){    return getRotationMatrixXYZ(Vector3<T>(ax,ay,az));}
 ...
 */
template<class T> Matrix3<T> normalizeRotationMatrix(const Matrix3<T>& R){
 ...
 */
Vector3<T> getEulerAngles(const Matrix3<T>& R){
    // rotation order
 ...
 */
bool isRotationMatrix(const Matrix3<T>& R){

    if(fabs((R.determinant()-1.0))>1e-7) return false;
    if((R.transpose()*R -Matrix3<T>(1,0,0,
                                    0,1,0,
 ...
 * @return
 */
bool isAlmostRotationMatrix(const Matrix3<T>& R){
 ...
 * @return
 */
Matrix3<T> getRotationMatrix(const Vector4<T>& q){
    ...
    T a=1.0 -2.0*(cc + dd); T b=T(2.0) * (bc - ad); T c=  T(2.0) * (ac + bd);
    T d=T(2.0) * (ad + bc); T e=1.0-2.0*( bb + dd);  T f=T(2.0) * (cd - ab);
    T g=T(2.0) * (bd - ac); T h=T(2.0) * (ab + cd); T i=1.0 - 2.0*( bb + cc);
...
Vector4<T> getRotationQuaternion(const Matrix3<T>& R){
    ...
    //T err=std::abs((q.length()-1));
    //if(err>1e-4) std::cout<<err<<std::endl;
...
template<class T> Matrix<T,4,4> QuaternionProductMatrix(Vector<T,4> q){
    return Matrix<T,4,4>(
                q(0),           -q(1),        -q(2),        -q(3),
                q(1),            q(0),        -q(3),         q(2),
                q(2),           q(3),         q(0),        -q(1),
